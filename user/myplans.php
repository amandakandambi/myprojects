<!DOCTYPE html>
<html>

<head>
<meta charset="utf-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>Home</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" type="text/css" media="screen" href="myplans.css" />
    <style>
        body {
          background-image: url('images/back.jpg');
        }
        </style>
</head>

<body>

<div class="navbar">


  <a href="register.php"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="100" height="50" viewBox="0 0 188 74">
    <defs>
      <filter id="Rectangle_2" x="0" y="0" width="188" height="74" filterUnits="userSpaceOnUse">
        <feOffset dy="3" input="SourceAlpha"/>
        <feGaussianBlur stdDeviation="3" result="blur"/>
        <feFlood flood-opacity="0.161"/>
        <feComposite operator="in" in2="blur"/>
        <feComposite in="SourceGraphic"/>
      </filter>
    </defs>
    <g id="Group_1" data-name="Group 1" transform="translate(-1612 -70)">
      <g transform="matrix(1, 0, 0, 1, 1612, 70)" filter="url(#Rectangle_2)">
        <g id="Rectangle_2-2" data-name="Rectangle 2" transform="translate(9 6)" fill="#7e9346" stroke="#707070" stroke-width="1">
          <rect width="170" height="56" rx="28" stroke="none"/>
          <rect x="0.5" y="0.5" width="169" height="55" rx="27.5" fill="none"/>
        </g>
      </g>
      <text id="Sign_Up" data-name="Sign Up" transform="translate(1660 114)" fill="#f2f3ed" font-size="30" font-family="OpenSans-SemiBold, Open Sans" font-weight="600"><tspan x="0" y="0">Sign Up</tspan></text>
    </g>
  </svg>
  </a><br>

  <a href="login.php">Login</a>
  <a href="about.html">About Us</a>
  <a href="myplans.php">My Plans</a>
  <a href="planmeal.php">Plan Meals</a>

<div id="logo">
    <a href="home.html">
  <svg xmlns="http://www.w3.org/2000/svg" width="150" height="50" viewBox="0 0 231 100">
    <text id="_EAT_HEALTHY" data-name="    EAT
  HEALTHY" transform="translate(0 40)" fill="#7e9346" font-size="40" "><tspan x="0" y="0" xml:space="preserve">    EAT</tspan><tspan x="0" y="50">HEALTHY</tspan></text>
  </svg></a>
  </div>
  

</div>




<br><br><br><br><br><br><br><br>
<div class="main">

<div id="title">
    
    My Plan
  </div>
  
<br><br><br>
<div id="item4">
        <font color="white">
          <p style="font-size: 20px;">&emsp;&emsp;Lose weight, Non veg Meal Plan
          
          <hr width="950">
      <table border="0" cellpadding="10px;">
        <tr style="font-size: 24px;">
          <td style width="600">&emsp;&emsp;Breakfast</td>
          <td width="600">&emsp;&emsp;Lunch</td>
          <td width="500">&emsp;&emsp;Dinner</td>
    </tr>

    
    <tr style="font-size: 16px;">

      <td>
         Boiled Eggs&emsp;&emsp;&emsp;2<br>
         Calories&emsp;&emsp;&emsp;&emsp;&nbsp;130kcal<br><br>
         <img src="images/download.jfif" style="width: 200px; height: 150px;">

      </td>
      <td>
         Sweet potatoes&emsp;&emsp;2<br>
         Calories&emsp;&emsp;&emsp;&emsp;&nbsp;130kcal<br><br>
         <img src="images/download (2).jfif" style="width: 200px; height: 150px;">
      </td>
      <td>
          Yogart&emsp;&emsp;&emsp;2<br>
         Calories&emsp;&emsp;130kcal<br><br>
         <img src="images/download (1).jfif" style="width: 200px; height: 150px;">
      </td>
    </tr>
    <tr>
      <td width="600"></td>
          <td width="600"></td>
          <td width="500">
            <div id="item3"><font size="5px">Reminder Set</font></div>
          </td>
    </tr>
    
  </font>
      </table>
      </font>
      <br>
      
  </div>
  <div id="title">
    <br>
    My Meal Plans
  </div>
  
<br><br><br>
<div id="item1">
        <font color="white">
          <p style="font-size: 20px;">&emsp;&emsp;Lose weight, Non veg Meal Plan</p>
          <hr width="950">
      <table border="0" cellpadding="10px;">
        <tr style="font-size: 24px;">
          <td style width="600">&emsp;&emsp;Breakfast</td>
          <td width="600">&emsp;&emsp;Lunch</td>
          <td width="500">&emsp;&emsp;Dinner</td>
    </tr>

    
    <tr style="font-size: 16px;">

      <td>
         Boiled Eggs&emsp;&emsp;&emsp;2<br>
         Calories&emsp;&emsp;&emsp;&emsp;&nbsp;130kcal<br><br>
         <img src="images/download.jfif" style="width: 200px; height: 150px;">

      </td>
      <td>
         Sweet potatoes&emsp;&emsp;2<br>
         Calories&emsp;&emsp;&emsp;&emsp;&nbsp;130kcal<br><br>
         <img src="images/download (2).jfif" style="width: 200px; height: 150px;">
      </td>
      <td>
          Yogart&emsp;&emsp;&emsp;2<br>
         Calories&emsp;&emsp;130kcal<br><br>
         <img src="images/download (1).jfif" style="width: 200px; height: 150px;">
      </td>
    </tr>
    <tr>
      <td width="600"></td>
          <td width="600"></td>
          <td width="500">
            <div id="item5"><font size="5px">Add as Your Plan</font></div>
          </td>
    </tr>
    
  </font>
      </table>
      </font>
      <br>
      </div></p>
      <br><br>
  <div id="item1">
        <font color="white">
          <p style="font-size: 20px;">&emsp;&emsp;Lose weight, Non veg Meal Plan</p>
          <hr width="950">
      <table border="0" cellpadding="10px;">
        <tr style="font-size: 24px;">
          <td style width="600">&emsp;&emsp;Breakfast</td>
          <td width="600">&emsp;&emsp;Lunch</td>
          <td width="500">&emsp;&emsp;Dinner</td>
    </tr>

    
    <tr style="font-size: 16px;">

      <td>
         Boiled Eggs&emsp;&emsp;&emsp;2<br>
         Calories&emsp;&emsp;&emsp;&emsp;&nbsp;130kcal<br><br>
         <img src="images/download.jfif" style="width: 200px; height: 150px;">

      </td>
      <td>
         Sweet potatoes&emsp;&emsp;2<br>
         Calories&emsp;&emsp;&emsp;&emsp;&nbsp;130kcal<br><br>
         <img src="images/download (2).jfif" style="width: 200px; height: 150px;">
      </td>
      <td>
          Yogart&emsp;&emsp;&emsp;2<br>
         Calories&emsp;&emsp;130kcal<br><br>
         <img src="images/download (1).jfif" style="width: 200px; height: 150px;">
      </td>
    </tr>
    <tr>
      <td width="600"></td>
          <td width="600"></td>
          <td width="500">
            <div id="item5"><font size="5px">Add as Your Plan</font></div>
          </td>
    </tr>
    
  </font>
      </table>
      </font>
      <br>
      </div>
  </div>

</div>

</body>
</html>
